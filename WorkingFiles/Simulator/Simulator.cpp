...
void Simulator::run(py::object simulate_function) {
    ...
    for (int iMacroStep = 0; iMacroStep < iMacroStepsPerSim; iMacroStep++) {
        if (bVerbose) {
            cout << "Beginning macro step " << iMacroStep + 1 << " of " << iMacroStepsPerSim << endl;
        }
...
void Simulator::reset() {
    ...
    try {
        ...
        throw std::runtime_error(std::string("Error retrieving current simulation history: ") + e.what());
    }
...
void Simulator::reset_economy() {
    ...
    try {
        ...
        throw std::runtime_error(std::string("Error resetting economy: ") + e.what());
    }
}
        ...
        for (const auto& agentData : this->simulatorConfigs["ai_agents"]) {
            if (agentData["agent_type"] == "stable_baselines_3") {
                if (agentData["production_policy"] == "Cournot") {
                    ...
                    throw std::runtime_error("AI agent production type not yet supported");
                }
...
void Simulator::shuffle_agent_firm_assignments() {
    throw std::runtime_error("Called shuffle agent-firm assignments option.\nThis has not been implemented yet because the simulator currently creates identical firms that begin with the same capital and no capabilities.\nCome back and fill in this function or turn the shuffle option off.");
}
...
void Simulator::set_agent_turn_order() {
    if (bVerbose) {
        cout << "Setting the agent turn order" << endl;
    }
...
void Simulator::perform_micro_step_control_agent_or_skip_turn(const int& iActingAgentID) {
    if (bVerbose) {
        cout << "Performing micro step. Acting agent ID: " << iActingAgentID << endl;
    }
    ...
    perform_micro_step_helper(vecActions);
}
...
void Simulator::perform_micro_step_helper(const vector<Action>& vecActions) {
...
vector<Action> Simulator::get_actions_for_all_agents_control_agent_turn(const int& iActingAgentID) {
    if (bVerbose) {
        cout << "Getting actions for all agents" << endl;
    }
    vector<Action> vecActions;
    for (const auto& pair : mapAgentIDToAgentPtr) {
...
void Simulator::execute_actions(const vector<Action>& vecActions, map<int, double>* pMapFirmIDToCapitalChange) {
    if (bVerbose) {
        cout << "Executing agent actions" << endl;
    }

    for (const Action& action : vecActions) {
        switch (action.enumActionType) {
            case ActionType::enumEntryAction:
                execute_entry_action(action, pMapFirmIDToCapitalChange);
                break;
            case ActionType::enumExitAction:
                execute_exit_action(action, pMapFirmIDToCapitalChange);
                break;
            ...
            default:
                ...
                throw std::runtime_error("Invalid enumerated action type");
        } // End of switch block
    } // End of for loop
}
...
Action Simulator::get_exit_action(const ControlAgent& agent) {
    ...
    // If exit policy is LOSS, choose the market with the lowest profit in the most recent time step
        ...
        for (int iMarketID : firmPtr->getSetMarketIDs()) {
            ...
            double dbRev = dataCache.mapFirmMarketComboToRevenue.at(pairFirmMarket);
            double dbFixedCost = dataCache.mapFirmMarketComboToFixedCost.at(pairFirmMarket);
            double dbVarCost = dataCache.mapFirmMarketComboToVarCost.at(pairFirmMarket);
            double dbQty = dataCache.mapFirmMarketComboToQtyProduced.at(pairFirmMarket);
        ...
        // Should never reach this part of the code
        throw std::runtime_error("Error getting exit action");
    }
...
void Simulator::init_simulation_history() {
    if (bVerbose) {
        cout << "Initializing simulation history" << endl;
    }
...
void Simulator::init_data_cache(SimulationHistory* pCurrentSimulationHistory) {
    if (bVerbose) {
        cout << "Initializing data cache" << endl;
    }
    ...
    try {
        ...
        for (auto combination : setFirmMarketCombinations) {
            ...
            double dbEntryCost = pCurrentSimulationHistory->mapMarketMaximumEntryCost.at(iMarketID);
        ...
        throw std::runtime_error(std::string("Error initializing the data cache: ") + e.what());
    }
}
...
BaseAgent* Simulator::get_agent_ptr_from_firm_ID(int iFirmID) {
    ...
    throw std::runtime_error("Error in get_agent_ptr_from_firm_ID");
}
...
void Simulator::distribute_profits(map<int, double>* pMapFirmIDToCapitalChange) {
    if (bVerbose) {
        cout << "Distributing profits" << endl;
    }
    ...
    for (const auto& market : economy.get_vec_markets()) {
        ...
        */
        ...
        for (int iFirmID : get_firm_IDs_in_market(market)) {
            ...
            double v = mapFirmIDToVarCosts.at(iFirmID);
            ...
            try {
                ...
                throw std::runtime_error(std::string("Error in distribute_profits method: ") + e.what());
            }
            ...
            if (policy == ProductionPolicy::Cournot) {
                ...
                throw std::runtime_error("Did not specify a valid production policy");
            }
...
bool Simulator::is_ai_agent(const int& iAgentID) {
    ...
    if (agentPtr->enumAgentType == AgentType::Control) {
        ...
        throw std::runtime_error(
                "Tried to determine whether agent was an AI agent but agent type has not been configured in"
                " Simulator::is_ai_agent. Add code to account for this agent type.");
    }
}
...
vector<double> Simulator::get_variable_cost_representation(const int& iAgentID) {
    ...
    for (int iMarketID : get_set_market_IDs()) {
        ...
        double dbVarCost = dataCache.mapFirmMarketComboToVarCost.at(pair);
...
vector<double> Simulator::get_fixed_cost_representation(const int& iAgentID) {
    ...
    for (int iMarketID : get_set_market_IDs()) {
        auto pair = std::make_pair(ptrFirmOfActingAgent->getFirmID(), iMarketID);
        double dbFixedCost = dataCache.mapFirmMarketComboToFixedCost.at(pair);
...
vector<double> Simulator::get_entry_cost_representation(const int& iAgentID) {
    ...
    for (int iMarketID : get_set_market_IDs()) {
        auto pair = std::make_pair(ptrFirmOfActingAgent->getFirmID(), iMarketID);
        double dbEntryCost = dataCache.mapFirmMarketComboToEntryCost.at(pair);
...
vector<double> Simulator::get_quantity_representation(const int& iAgentID) {
    ...
    for (int iMarketID : get_set_market_IDs()) {
        auto pair = std::make_pair(ptrFirmOfActingAgent->getFirmID(), iMarketID);
        double dbQty = dataCache.mapFirmMarketComboToQtyProduced.at(pair);
...
vector<double> Simulator::get_price_representation(const int& iAgentID) {
    ...
    for (int iMarketID : get_set_market_IDs()) {
        auto pair = std::make_pair(ptrFirmOfActingAgent->getFirmID(), iMarketID);
        double dbPrice = dataCache.mapFirmMarketComboToPrice.at(pair);
...
[[maybe_unused]] double Simulator::generate_reward(const int& iAgentID) {
    ...
    auto firmPtr = get_firm_ptr_from_agent_id(iAgentID);
    double dbCurrentCapital = firmPtr->getDbCapital();
    double dbPreviousCapital = mapAIAgentIDToCapitalAtLastTurn.at(iAgentID);
    int iPreviousMicroTimeStep = mapAIAgentIDToMicroTimeStepOfLastTurn.at(iAgentID);
