#!/usr/bin/env python3
"""Aggregate DQN hyperparameter sweep results into a single CSV summary."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Combine DQN sweep hyperparameters and evaluation metrics into a CSV table.",
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=Path("WorkingFiles/Sweeps/dqn_slurm"),
        help="Directory produced by submit_slurm_dqn_sweep.py (default: WorkingFiles/Sweeps/dqn_slurm).",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--slurm-jobs-dir",
        type=Path,
        default=Path("WorkingFiles/SlurmJobs/dqn"),
        help="Directory containing generated sbatch files to purge after aggregation (default: WorkingFiles/SlurmJobs/dqn).",
    )
    parser.add_argument(
        "--skip-cleanup",
        action="store_true",
